...
def main():
    ...
    for i in range(1):
        '''
        start_time = time()
        while time() - start_time < 9.16:
            kob.drive(thr=1, steer=0.5)
        kob.stop()
        sleep(1)
        '''
        start_heading = mag.get_heading()
        if i % 2 == 0:
            str_dir = 0
        else:
            str_dir = 1
        kob.drive(thr=1, steer=str_dir)
        sleep(1)
        while abs(mag.get_heading() - start_heading) > 1:
            kob.drive(thr=1, steer=str_dir)
        print('Start', start_heading, 'end', mag.get_heading())
        kob.stop()
        sleep(2)
    '''
    start_time = time()
    while time() - start_time < 13.3:
